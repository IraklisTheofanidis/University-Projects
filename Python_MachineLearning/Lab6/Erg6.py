# -*- coding: utf-8 -*-
"""
Created on Tue May 14 21:47:43 2019
...
def nbtrain(x,t):
  ...
  zeroPriorProb=0.0
  onesPriorProb=0.0
  attriCount=x.shape[1]
  totalCount=t.shape[0]
  #test=np.float64()
  for i in range(0,t.shape[0]):
    if t[i] == 0:
        #test[zerosCount]=x[i,0]
        zerosCount +=1 
    else:
        onesCount +=1

  zeroArray=np.ones((zerosCount,attriCount))
  oneArray=np.ones((onesCount,attriCount))
  mu=np.ones((2,attriCount))
  sigma=np.ones((2,attriCount))
 
  counterZeros=0;
  counterOnes=0;
  for k in range(0,t.shape[0]):
   if(t[k]==0):
    zeroArray[counterZeros]=x[k]
    counterZeros +=1
   else:
    oneArray[counterOnes]=x[k]  
    counterOnes+=1
  ...
  print mu
  print sigma
  zeroPriorProb=zerosCount/float(totalCount)
  onesPriorProb=onesCount/float(totalCount)
  prior=np.float64([zeroPriorProb,onesPriorProb])
  print "Zeros Probs: ",prior[0],prior[1]
  print "Zeros :",zerosCount,onesCount
  model["prior"]=prior
  model["mu"]=mu
  model["sigma"]=sigma
  return model
def nbpredict( xtest, model ):
  for p in range(0,xtest.shape[0]):
    l=model["prior"][1]/model["prior"][0]
    print "LLLLLL",l
    for i in range(0,xtest.shape[1]):
      l=l*(norm.pdf(xtest,loc=model["mu"],scale=["sigma"]))
...
while ans == 4:
    ...
    dic=nbtrain(xtrain,ttrain)
    nbpredict(xtest,dic)
    #print "aaaaaaaaaaaaaaaaaaaaaaaaaa",aa[0],aa[1]
